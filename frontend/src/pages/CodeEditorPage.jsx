...
export default function CodeEditorPage() {
  ...
  if (!problem)
    return <div className="p-5 text-center">Loading problem...</div>;
  ...
  return (
    <div className="page-container" style={{ minHeight: "calc(100vh - 4rem)" }}>
      <div className="d-flex flex-column flex-md-row editor-shell">
        ...
        <div
          className="w-100 overflow-auto p-4 border-end border-secondary problem-panel"
          style={{ width: "50%", maxHeight: "calc(100vh - 4rem)" }}
        >
          ...
          <div
            className="mb-5 text-light opacity-75"
            style={{ whiteSpace: "pre-wrap", fontFamily: "system-ui" }}
          >
            {problem.description}
          </div>
          ...
          {problem.examples && problem.examples.length > 0 && (
            <div className="mb-5">
              ...
              {problem.examples.map((ex, i) => (
                <div
                  key={i}
                  className="card example-card border-0 rounded-3 mb-3 font-monospace small shadow-sm"
                >
                  <p className="mb-1">
            ...
            <div>
              ...
              <ul className="list-unstyled card bg-secondary border-0 p-3 rounded-3 shadow-sm font-monospace small">
                {problem.constraints.map((c, i) => (
                  <li key={i} className="mb-1 text-warning">
                    • {c}
                  </li>
                ))}
              </ul>
        ...
        {/* Editor Panel */}
        <div
          className="w-100 d-flex flex-column editor-panel"
          style={{
            width: "50%",
        ...
        >
          ...
          <div
            className="flex-grow-1 code-area"
          ...
          {/* Output Panel */}
          <div
            className="output-panel overflow-auto position-relative"
            style={{
              minHeight: "250px",
          ...
          >
            <div className="p-3">
              <h3 className="h6 text-light opacity-50 fw-bold mb-3 text-uppercase">
                Execution Result
              </h3>
              {!result && (
              ...
              {result && result.error && (
                <p
                  className="text-danger font-monospace small"
              ...
              {result && result.submission && (
                <div>
                  <div
                    className={`h5 fw-bold mb-2 ${result.submission.status === "Accepted" ? "text-success" : "text-danger"}`}
                  >
                  ...
                  <div className="d-flex gap-4 small text-light opacity-75 mb-3">
                    <span>
                      ...
                      <strong className="text-light">
                        {result.submission.passedCases}/
                        {result.submission.totalCases}
                      </strong>
                    </span>
                    <span>
                      Time:{" "}
                      <strong className="text-light">
                        {result.submission.executionTime}s
                      </strong>
                    </span>
                  </div>
                  ...
                  {result.failedCaseInfo && (
                    <div className="alert alert-danger p-3 rounded-2 small font-monospace mt-2 border-0 bg-danger bg-opacity-10 text-danger">
                      {result.failedCaseInfo.error ? (
                        ...
                        <>
                          <div className="mb-2">
                            <span className="fw-bold opacity-75">Input:</span>
                            <br />
                            {result.failedCaseInfo.input}
                          </div>
                          <div className="mb-2">
                            ...
                            <span className="text-success">
                              {result.failedCaseInfo.expected}
                            </span>
                          </div>
                          <div>
                            <span className="fw-bold opacity-75">Actual:</span>
                            <br />
                            {result.failedCaseInfo.actual}
                          </div>
                        </>
                      )}
                    </div>
